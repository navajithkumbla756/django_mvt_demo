...
from .models import Candidate, CustomUser, Employer
...
def create_role_based_profile(sender, instance, created, **kwargs):
    ...
    if created:
        if instance.role == "EMPLOYER":
            Employer.objects.create(user=instance, company_name=f"{instance.name}'s Organization Workspace")
        elif instance.role == "CANDIDATE":
            Candidate.objects.create(user=instance, skills="Initial Registration State")
